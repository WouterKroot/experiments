...
class Window:
    ...
    def countdown(self, duration=3):
        ...
        while t.getTime() >= 0:
            cd = visual.TextStim(self.win, color = self.stimulus_colour, height = 32, text = f"Experiment will continue in {math.ceil(t.getTime())}...")
            self.drawOrder(cd)
            
    def intro_baseline(self):
        intro_msg = visual.TextStim(self.win, color= self.stimulus_colour, height = 32, wrapWidth=600,
    ...
    def midway(self,nBlocks):
        midway_msg = visual.TextStim(self.win, color= self.stimulus_colour, height = 32, wrapWidth=600,
    ...
    def end(self):
        m_end = visual.TextStim(self.win, color=self.stimulus_colour, height = 32, wrapWidth=600,
